...
component accessors="true" extends="../BaseRequest" {
	...
	public function get( required string task_uid ){
		return handleResponse(
			MeilisearchClient.get( "/tasks/#arguments.task_uid#" )
		);
	}
	 ...
	 */
	public function getAllByIndex(
        required string index_uid,
    ){
		return handleResponse(
			MeilisearchClient.get( "/indexes/#arguments.index_uid#/tasks" )
		);
	}
